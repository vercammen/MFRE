...
The various results matrices have been stored as .RDS files in the "/Code/SavedRDS" folder. The first one which corresponds to an EU supply intercept with value -1.0 can be loaded as follows:

```{r}
results_1.0 <- readRDS("./SavedRDS/results_1.0.RDS")
results_1.0
```
...
```{r}
results_0.9 <- readRDS("./SavedRDS/results_0.9.RDS")
results_0.9
```
...
```{r}
results_0.8 <- readRDS("./SavedRDS/results_0.8.RDS")
results_0.8
```
...
```{r}
results_0.7 <- readRDS("./SavedRDS/results_0.7.RDS")
results_0.7
```
